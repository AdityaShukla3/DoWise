...
// Generate Similar Technologies
exports.getSimilarTechnologies = async (req, res) => {
  try {
    ...
    if (useOpenAI && key) {
      const prompt = `Suggest 3 to 5 technologies, tools, or subjects that are similar or highly related to "${technology}".
Return ONLY a JSON array of objects with the structure:
[
  { "name": "Related Tech Name", "reason": "Brief reason why it's related" }
]`;
      ...
      const resp = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "gpt-3.5-turbo",
          messages: [
            { role: "system", content: "You are an expert tech recommender. Return ONLY valid JSON." },
            { role: "user", content: prompt }
          ],
          temperature: 0.5,
          max_tokens: 500
      ...
      if (parsed && Array.isArray(parsed) && parsed.length > 0) {
        return res.json({ similar: parsed });
      }
    }
    
    // Fallback if no AI or it fails
    res.json({ 
      similar: [
        { name: "Related Tool 1", reason: `A great next step after ${technology}` },
        { name: "Related Tool 2", reason: "Commonly used in the same stack" },
        { name: "Related Tool 3", reason: "A popular alternative" }
      ]
    });
  } catch (err) {
    console.error("Similar tech error", err);
...
exports.getQuiz = async (req, res) => {
  try {
    ...
    if (useOpenAI && key) {
      ...
      const prompt = `Create a multiple-choice quiz with exactly 25 questions to evaluate a user's knowledge on the course "${plan.rawInput}".
...
Ensure the questions range from beginner to intermediate difficulty.`;
    ...
    const fallbackQuiz = plan.tasks.slice(0, 25).map((t, idx) => ({
...
// Generate MCQ Quiz for any Topic (Quick Quiz)
exports.quickQuiz = async (req, res) => {
  try {
    ...
    if (useOpenAI && key) {
      const prompt = `You are a technical evaluator. First, evaluate if "${cleanTopic}" is a valid software, hardware, programming language, database, cloud, devops, AI/ML, framework, computer science, or IT technology topic. 
If it is NOT a technology topic (e.g. if it is a general knowledge topic like "bathroom", "spider", "cooking", "history", "fishing", "music"), return exactly this JSON and nothing else:
{"error": "Please enter a valid technology topic (e.g. React, Python) for revision. Non-tech topics like '${cleanTopic}' are not supported."}

If it IS a valid technology topic, create a multiple-choice quiz with exactly 25 questions to evaluate a user's knowledge on "${cleanTopic}".
...
Return the output ONLY as JSON (do not include markdown syntax, code blocks, or extra text).
Final structure should be either:
{"error": "..."}
OR
{"quiz": [ ... ]}`;
      ...
      const resp = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "gpt-3.5-turbo",
          messages: [
            { role: "system", content: "You are a technical evaluator. You validate technology topics and generate quizzes. Return ONLY JSON." },
            { role: "user", content: prompt }
          ],
          temperature: 0.4,
          max_tokens: 3000
    ...
    // Fallback static validation / quiz if offline or OpenAI fails
    const techName = cleanTopic.toLowerCase();
    const nonTechList = ["bathroom", "spider", "fishing", "history", "cooking", "sports", "art", "music", "dog", "cat", "car"];
    if (nonTechList.some(nt => techName.includes(nt))) {
      return res.status(400).json({ message: `Please enter a valid technology topic (e.g. React, Python) for revision. Non-tech topics like '${cleanTopic}' are not supported.` });
    }

    // Generate static tech quiz for fallback
    const fallbackQuiz = Array.from({ length: 25 }, (_, idx) => ({
      question: `Question ${idx + 1}: Which of the following is a core concept of ${cleanTopic}?`,
      options: [
        `Syntax and semantics of ${cleanTopic}`,
        `Unrelated theories of ${cleanTopic}`,
        `A random guess about ${cleanTopic}`,
        `None of the above`
      ],
      correctAnswerIndex: 0,
      topic: cleanTopic
    ...
    console.error("Quick quiz generation error", err);
    res.status(500).json({ message: "Failed to generate quick quiz" });
  }
};

// Evaluate Quick Quiz Answers
exports.quickQuizEvaluate = async (req, res) => {
  try {
    const { answers, quiz, topic } = req.body;
    if (!answers || !quiz || !topic) return res.status(400).json({ message: "answers, quiz and topic are required" });
    ...
    if (useOpenAI && key) {
      ...
      const prompt = `A student completed a quick quiz for the topic "${topic}".
Total Score: ${score} out of ${quiz.length}.
Topics they answered correctly: ${correctList || 'None'}.
Details of questions they answered incorrectly:
${incorrectList || 'None (Perfect Score!)'}
...
Limit improvementTopics to maximum 3 items, representing specific areas they should review.`;
      ...
      const resp = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "gpt-3.5-turbo",
          messages: [
            { role: "system", content: "You are an expert tutor. Evaluate the quick quiz results and provide structured feedback and topic improvement suggestions in JSON." },
            { role: "user", content: prompt }
          ],
          temperature: 0.4,
          max_tokens: 800
    ...
    console.error("quick quiz evaluation error", err);
    res.status(500).json({ message: "Failed to evaluate quick quiz" });
  }
};
