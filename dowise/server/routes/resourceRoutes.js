...
router.post("/search", async (req, res) => {
  try {
    ...
    if (techKey && technologyResources[techKey]) {
      ...
      if (useOpenAI && key) {
        const prompt = `You are an expert technology learning path designer. For the technology "${technology}", provide:
...
4. Difficulty level
5. Prerequisites
6. A vertical roadmap containing 4-5 core milestone learning steps
7. Three projects (one Beginner, one Intermediate, one Advanced)
8. Three expert AI mentor tips
9. Five skills gained after completing this plan
10. Three career opportunities/job roles for this technology (role name, salary estimate, demand growth percentage)
        ...
        try {
          const resp = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
              model: "gpt-3.5-turbo",
              messages: [
                { role: "system", content: "You are an expert technology learning path designer. Provide accurate, realistic learning resources and time estimates in the requested JSON structure." },
...
function enrichTechnologyData(techName, baseData) {
  ...
  if (!careerOpportunities || !careerOpportunities.length) {
